...
export default function Home() {
  ...
  const [isCheckingPhrase, setIsCheckingPhrase] = useState(true);
  ...
  if (isCheckingPhrase) return null;

  const handlePhraseSubmit = async () => {
    try {
      setIsLoading(true);
      const uuid = uuidv4();
      const existingUserId = await insertUser({
        phrase: loginPhrase,
        userId: uuid,
      });
      if (existingUserId !== null) {
        login({ phrase: loginPhrase, userId: existingUserId });
      } else {
        login({ phrase: loginPhrase, userId: uuid });
      }
    } catch (error: any) {
      toast.error(error.message, { position: "top-center" });
    } finally {
        ...
        <form className="space-y-6">
          <div className="space-y-3">
            <div className="space-y-2">
              ...
              <Input
                type="text"
                placeholder="Enter your login phrase"
                value={loginPhrase}
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Note: Enter literally anything. This is just to identify you. Make
              sure to remember it!! 🚀
            </p>
          </div>
          <Button
            ...
            disabled={isLoading || !loginPhrase}
            className="bg-[#2997FF] hover:bg-[#147CE5] text-white font-semibold py-2 px-6 rounded-lg transition-colors w-full"
          >
            {isLoading ? (
              <Spinner />
            ) : (
...
//           <li>Enter a purpose (optional) and leave the expiry date blank</li>
